    ...
    try {
        ...
        if (data.success && data.data) {
            if (data.data.topSongs && data.data.topSongs.length > 0) {
                console.log("[Artist] First song raw fields:", Object.keys(data.data.topSongs[0]));
                console.log("[Artist] First song playCount:", data.data.topSongs[0].playCount, "play_count:", data.data.topSongs[0].play_count);
                data.data.topSongs = data.data.topSongs.map(s => ({ ...s, playCount: s.playCount ?? s.play_count ?? 0 }));
            }
            return { json: () => data };
        }
            ...
            if (songsData.data && songsData.data.songs && songsData.data.songs.length > 0) {
                ...
                const reconstructedData = {
                    ...
                    data: {
                        ...
                        dominantType: "Artist",
                        bio: [],
                        topSongs: songsData.data.songs.map(s => ({ ...s, playCount: s.playCount ?? s.play_count ?? 0 })),
